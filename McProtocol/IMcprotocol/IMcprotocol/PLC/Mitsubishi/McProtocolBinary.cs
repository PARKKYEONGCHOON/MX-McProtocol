...
namespace InControls.PLC.Mitsubishi
{
	...
	abstract public class McProtocolBinary : IMitsubishiPlc
    {
        ...
        public int WriteDeviceBlock(PlcDeviceType iType, int iAddress, int iSize, int[] iData)
        {
            ...
            byte[] sdCommandMC3E = Command.SetCommandMC3E(0x1401, 0x0000, data.ToArray());
            byte[] rtResponse = TryExecution(sdCommandMC3E);
        ...
        //    byte[] sdCommandMC3E = Command.SetCommandMC3E(0x0401, 0x0000, data.ToArray());
        //    byte[] rtResponse = TryExecution(sdCommandMC3E);
        //    int rtCode = Command.SetResponse(rtResponse);
        //    byte[] rtData = Command.Response;
        //    for (int i = 0; i < iSize; ++i)
        ...
        public int SetDevice(PlcDeviceType iType, int iAddress, int iData)
        {
            ...
            byte[] sdCommand = Command.SetCommandMC4E(0x1401, 0x0000, data.ToArray());
            int length = 15;
            byte[] rtResponse = TryExecution(sdCommand, length);
            int rtCode = Command.SetResponseWrite(rtResponse);
            return rtCode;
        }
        // ====================================================================================
        ...
        private byte[] TryExecution(byte[] iCommand, int minlength)
        {
            ...
            do
            {
                ...
                if (tCount < 0)
                {
                    ILogger.Add(LOG_TYPE.ERROR, "[Failed] --> Send Message(Checking The Command");
                    ILogger.Add(LOG_TYPE.ERROR, "[OK] {0}==>{1}", MethodBase.GetCurrentMethod().ReflectedType.Name, MethodBase.GetCurrentMethod().Name);
                    return rtResponse;
                }
